...
SRCS := $(CADR_SRCS) $(CADR1_SRCS) $(DIP_SRCS) $(HELPER_SRCS) $(TTL_SRCS) $(CADR_TB_SRC) $(HELPER_VHD)
...
$(HELPER_VHD):
	mkdir -p $(BUILDDIR)
	python3 $(CREATE_PACKAGE_FROM_ENTITIES_PY) -p $@ -e helper/helper_*.vhd

# generate cadr_tb.vhd using SUDS and helper components
$(CADR_TB_SRC): $(CREATE_TB_PY) cadr/cadr_book.vhd cadr/icmem_book.vhd cadr1/busint_book.vhd $(HELPER_VHD)
	mkdir -p $(BUILDDIR)
	python3 $< \
	--vhdl-files cadr/cadr_book.vhd cadr/icmem_book.vhd cadr1/busint_book.vhd $(HELPER_VHD) \
...
help: 
	...
	@echo "make regen: regenerate (fast-promh-cadr and cadr1)"
