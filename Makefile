...
# cadr_tb and cadr_cosim_tb is autogenerated by create-tb.py on every build
# this is without cosimulation
CADR_TB_SRC := build/cadr_tb.vhd
CADR_TB_EXE := build/cadr_tb

# all sources and executables
SRCS := $(CADR_SRCS) $(ICMEM_SRCS) $(BUSINT_SRCS) $(DIP_SRCS) $(HELPER_SRCS) $(TTL_SRCS) $(CADR_TB_SRC) $(COSIM_CADR_TB_SRC) $(COSIM_SRCS)
EXES := $(CADR_EXES) $(ICMEM_EXES) $(BUSINT_EXES) $(DIP_EXES) $(HELPER_EXES) $(TTL_EXES) $(CADR_TB_EXE) $(COSIM_CADR_TB_EXE) $(COSIM_EXES)
