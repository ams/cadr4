...
COSIM_CSRCS := $(wildcard $(COSIM_DIR)/*.c)
COSIM_COBJS := $(BUILDDIR)/$(notdir $(patsubst %.c,%.o,$(COSIM_CSRCS)))
COSIM_VPI 	:= $(BUILDDIR)/cosim.vpi

# cadr_tb
CADR_TB_SRC := build/cadr_tb.vhd
CADR_TB_EXE := build/cadr_tb
# cadr_boot_tb auto boots
CADR_BOOT_TB_SRC := build/cadr_boot_tb.vhd
CADR_BOOT_TB_EXE := build/cadr_boot_tb

# all sources and executables
SRCS := $(CADR_SRCS) $(ICMEM_SRCS) $(BUSINT_SRCS) $(DIP_SRCS) $(HELPER_SRCS) $(TTL_SRCS) $(CADR_TB_SRC) $(CADR_BOOT_TB_SRC) $(COSIM_SRCS)
EXES := $(CADR_EXES) $(ICMEM_EXES) $(BUSINT_EXES) $(DIP_EXES) $(HELPER_EXES) $(TTL_EXES) $(CADR_TB_EXE) $(CADR_BOOT_TB_EXE) $(COSIM_EXES)
...
$(COSIM_COBJS): $(COSIM_CSRCS)
	mkdir -p $(BUILDDIR)
	$(GHDL) --vpi-compile -v clang -c $< -o $@

# link cosim objects to cosim.vpi
$(COSIM_VPI): $(COSIM_COBJS)
	mkdir -p $(BUILDDIR)
	$(GHDL) --vpi-link -v clang $(COSIM_COBJS) -o $@
...
# targets calling the ones above
.PHONY: cosim-run-%
cosim-run-%: $(BUILDDIR)/%_tb
	TB=$< make cosim-run-tb
...
print-config:
	...
	$(info SRCS: $(SRCS))
	$(info EXES: $(EXES))
	$(info COSIM_CSRCS: $(COSIM_CSRCS))
	$(info COSIM_COBJS: $(COSIM_COBJS))
	$(info COSIM_VPI: $(COSIM_VPI))
