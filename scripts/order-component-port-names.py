#!/usr/bin/env python3
"""
Script to order component port names alphabetically in VHDL package files.
...
def parse_component_ports(component_text: str) -> Tuple[str, List[str], List[str]]:
    """
    Parse a component declaration and extract port information.
    Returns (component_name, port_lines, non_port_lines).
    """
    lines = component_text.strip().split('\n')
    
    # Find component name
    component_match = re.search(r'component\s+(\w+)', lines[0], re.IGNORECASE)
    ...
    return component_name, port_lines, non_port_lines
...
def format_component(component_name: str, port_lines: List[str], non_port_lines: List[str]) -> str:
    """
    Format a component with sorted ports.
    """
    if not port_lines:
    ...
    # Add lines before port declaration (component declaration)
    for line in non_port_lines:
        if 'end component' in line.lower():
            break
...
def process_vhdl_file(content: str) -> str:
    """
    Process a VHDL file and sort component port names.
    """
    # Find all component declarations more carefully
    component_pattern = r'(\n\s*component\s+\w+.*?end\s+component\s*;)'
    
    def replace_component(match):
        component_text = match.group(1)
        component_name, port_lines, non_port_lines = parse_component_ports(component_text)
        formatted = format_component(component_name, port_lines, non_port_lines)
        
        # Preserve the leading newline and original indentation
        lines = component_text.split('\n')
        first_content_line = next((line for line in lines if line.strip().startswith('component')), '')
    ...
    # Replace each component with sorted version
    result = re.sub(component_pattern, replace_component, content, flags=re.DOTALL | re.IGNORECASE)
    
    return result


def main():
    parser = argparse.ArgumentParser(description='Order component port names alphabetically in VHDL package files')
    parser.add_argument('file', help='VHDL package file to process')
