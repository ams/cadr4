...
-- doc/ttl/dm9328.pdf
--
-- The clock of each register is the OR of its separate clock input and the
-- common clock input (pin 9). A register shifts on the low-to-high transition
-- of that OR, i.e. a high level on either clock input inhibits the other.
...
architecture behavioral of dm9328 is
  ...
  -- effective clocks: OR of the separate and the common clock
  signal aclk_or, bclk_or : std_logic;
...
begin
  ...
  aclk_or <= aclk_i or comclk_i;
  bclk_or <= bclk_i or comclk_i;
  ...
  begin
    if clr_n_i = '0' then
      rega <= (others => '0');
    elsif rising_edge(aclk_or) then
      if asel_i = '0' then
  ...
  begin
    if clr_n_i = '0' then
      regb <= (others => '0');
    elsif rising_edge(bclk_or) then
      if bsel_i = '0' then
